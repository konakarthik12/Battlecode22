...
import static Aj2.Constants.directions;
...
class Builder {
    ...
    static void setDestination(RobotController rc) throws GameActionException {
        if (destination == null) {
            int lowX = Math.max(spawn.x - 10, 0);
            int highX = Math.min(spawn.x + 10, rc.getMapWidth());
            int lowY = Math.max(spawn.y - 10, 0);
            int highY = Math.min(spawn.y + 10, rc.getMapHeight());
