...
class Soldier {
    ...
    static void setDestination(RobotController rc) throws GameActionException {
        if(visibleAllies <= 5 && visibleEnemies == 0) {
            rc.setIndicatorString("following");
            int minID = rc.getID();
            for (RobotInfo robotInfo : rc.senseNearbyRobots(-1, rc.getTeam())) {
                if(robotInfo.getType().equals(RobotType.SOLDIER) && rc.getID() < minID){
                    minID = rc.getID();
    ...
    static void act(RobotController rc) throws GameActionException {
        ...
        if (visibleEnemies > 0) {
            ...
            if(rc.getRoundNum() - spawnTurn >= 10  || visibleAllies >= 2 || rc.getMapHeight() * rc.getMapWidth() <= 1600) Pathfinder.move(rc, destination);
            attack(rc);
        }
    }
