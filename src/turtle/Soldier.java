...
//            }
        ...
        if (destination != null && isBackup) {
            int x = Math.min(Math.max(0, destination.x + dx), Utils.width);
            int y = Math.min(Math.max(0, destination.y + dy), Utils.height);
            destination = new MapLocation(x, y);
        }

//            destination = new MapLocation(destination.x + dx, destination.y + dy);
    }
